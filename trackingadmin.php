...
<head>
	...
	<title>Tracking | Logistics Association</title>
</head>
<body>
	<!-- Navigation Bar-->
...
<div class="container">
    <div class="row height d-flex justify-content-center align-items-center">
        <div class="col-md-8">
            ...
            <div class="search"> <i class="fa fa-search"></i> <input type="text" class="form-control" placeholder="Tracking number...." name="tracking_number"> <button class="btn btn-primary" name="submit">Search</button> </div>
        	<div class="note text-center">E.g. : TH1234HH5FH6,MYMP000001123</div>
        	</form>
        </div>
    </div>
    <br><br><br>
...
$db = mysqli_select_db($con, 'tracking');

if(isset($_POST['submit']))
{
	$tracking_number = $_POST['tracking_number'];

	$query = "SELECT * FROM track where tracking_number = '$tracking_number' ";
	$query_run = mysqli_query($con,$query);
	while($row = mysqli_fetch_array($query_run))
	{
		?>
		<div class="detail">
			<div class="title"><h3>Tracking Detail</h3></div>
			<br>
			<div class="data">
			<form action="" method="POST">	
				<input type="hidden" name="Tracking_id" value="<?php echo $row['Tracking_id'] ?>" readonly/>
				<input type="text" name="Tracking_number" value="<?php echo $row['Tracking_number'] ?>" readonly/>
				...
				<input type="text" name="States" value="<?php echo $row['Status'] ?>" readonly/>
				<label>Status</label>
				<input type="text" name="Ship_date" value="<?php echo $row['Ship_date'] ?>" readonly/>
				<label>Ship Date</label>
				<input type="text" name="Expected_ship_date" value="<?php echo $row['Expected_ship_date'] ?>" readonly/>
